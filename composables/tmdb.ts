...
import type { Credits, Media, MediaType, PageResult, Person, TVMedia } from '../types'
...
export function fetchTMDB<T>(url: string, params: Record<string, string | number | boolean | undefined> = {}): Promise<T> {
...
export async function getMedia(type: MediaType, id: string) {
	...
	return fetchTMDB<Media | TVMedia>(`${type}/${id} `, {
 ...
 * Get recommended
 */
export async function getRecommendations(type: MediaType, id: string, page = 1): Promise<PageResult<MediaType extends 'tv' ? TVMedia : Media>> {
